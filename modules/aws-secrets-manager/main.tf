...
# ─────────────────────────────────────────
# Kong Manager UI login (basic-auth in front of the Kong Manager UI)
# Separate path/secret per the app team's ExternalSecret kong-manager-basic-auth,
# which reads <env>/zord/kong-manager -> KONG_MANAGER_PASSWORD. Auto-generated by
# Terraform (no CHANGE_ME, no on-pod generation), unique per environment. Covered
# by the existing ESO wildcard scope <env>/zord/*, so no new IAM is required.
# ─────────────────────────────────────────

resource "random_password" "kong_manager" {
  length  = 24
  special = false
}

resource "aws_secretsmanager_secret" "kong_manager" {
  name                    = "${var.environment}/zord/kong-manager"
  description             = "Kong Manager UI basic-auth login password (auto-generated) for Arealis Zord (${var.environment})"
  recovery_window_in_days = 0
  tags                    = { Name = "${var.environment}/zord/kong-manager" }
}
